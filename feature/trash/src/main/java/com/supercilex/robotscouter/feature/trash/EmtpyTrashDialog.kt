...
internal class EmtpyTrashDialog : DialogFragmentBase(), DialogInterface.OnClickListener {
    ...
    override fun onClick(dialog: DialogInterface, which: Int) {
        emptyTrash(if (emptyAll) null else ids)

        longSnackbar(checkNotNull(parentFragment?.view), resources.getQuantityString(
                R.plurals.trash_emptied_message, ids.size, ids.size))
    }
