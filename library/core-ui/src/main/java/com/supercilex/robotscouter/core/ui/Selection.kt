...
abstract class MenuHelperBase<T>(
    ...
    private fun updateToolbarColor(normal: Boolean) {
        ...
        if (
            toolbar.background.run {
                this !is ColorDrawable || color != ContextCompat.getColor(activity, newColorPrimary)
            }
        ) animateColorChange(
...
abstract class DrawerMenuHelperBase<T>(
    ...
    override fun shouldUpdateStatusBarColor(new: Int): Boolean {
        ...
        return drawable !is ColorDrawable || drawable.color != ContextCompat.getColor(activity, new)
    }
